const Input = ({ label, error, helper, className = '', ...props }) => (
  <label className="block">
    ...
    <input
      className={`focus-ring w-full rounded-[1.15rem] border border-sky-100 bg-white/95 px-4 py-3 text-sm font-semibold text-slate-900 shadow-sm placeholder:text-slate-400 ${className}`}
      {...props}
    />
